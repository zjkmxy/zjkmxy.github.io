...
date: 2020-12-10
permalink: /posts/2020/12/polynomials/
tags:
  - algorithm
  - algebra
---
...
Let $R$ be a UFD and $f\in R[[ X]]$ be a power series.
Suppose $f(a)\in Rp$ for some $a,p\in R$, i.e. $a$ is a root to $f$ modulo $p$.
Then, consider $x = a+rp$.
We have $f(a) + f'(a)rp \in Rp^2$.
Since $p\mid f(a)$, if $f'(a)$ is invertible in $R/p^2R$, we have $p\mid f(a)f'(a)^{-1}$.
Thus, $x = a - f(a)f'(a)^{-1}$ is a root to $f$ modulo $p^2$ if $f'(a)$ is invertible.
...
Let $p_0(X) = \prod_{i=1}^{n/2} (X - x_i)$ and $p_1(X) = \prod_{i=n/2+1}^{n} (X - x_i)$.
Define $r_0(X) = f\mod p_0$ and $r_1(X) = f\mod p_1$.
Then it suffices to evaluate $r_0$ on $x_1,\ldots, x_{n/2}$ and $r_1$ on $x_{n/2+1},\ldots, x_{n}$.
