...
Then the right boundary will be monotonically increasing, and the real distance -- that we need to go back and forth --
...
The output of the algorithm is the search result -- a list of webpages sorted by the weighted average of likeliness scores.
...
- If we handle them differently, we can have $$sqrt{KCQN})$$ by the sqrt thesis.
...
### Variant
One variant is to allow modifications -- change the likeliness score $$L_{i,j}$$ to an input number.
