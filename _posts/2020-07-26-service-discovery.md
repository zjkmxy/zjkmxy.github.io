...
- **Sequential consistency**: All writes are globally ordered and seen in the order of happening.
  - E.g. if a node see data's changing in this way `(X=0,Y=0) -> (X=5,Y=0) -> (X=5,Y=10)`,
...
- **Linearizability**: When a read finishes, it must return the latest state of an object.
  - E.g. if a node starts a read request at `t1` and it finishes at `t2`, then it must contain the newest data before `t2`.
    If another node sends an update request which finishes between `t1` and `t2`, it must be reflected.
...
In CAP theorem, a strong consistent database is a AP system.
...
If all controllers are down, the endpointslices "freeze" -- they won't reflect new changes of pods' IPs.
...
This only solves part of the problem -- the load balancing part. The service registration problem is still there.
