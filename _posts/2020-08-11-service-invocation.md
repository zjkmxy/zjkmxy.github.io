...
date: 2020-08-11
permalink: /posts/2020/08/service-invocation/
tags:
  - ndn
  - service-invocation
  - api
...
Data to cache and expiry time are managed by the backend services -- just like a in-memory version of NDN Repo.
...
![Transaction](/images/service-invocation/transaction.png)
...
But this only delegates the problem without solving it -- after all, the message queue system needs to worry about this.
...
However, the complexity cannot be eliminated -- it's delegated to the service mesh developers.
...
- "Unfortunate architectural decisions around Mixer":
  ...
  However, this doubles every service call -- the sidecar needs to contact Mixers first and then services.
